 ...
 *     			separatley.
 */
function caEnablePanicOnCrash(filepath, file, options)
{
	...
	process.on('uncaughtException', function (ex) {
	    caPanic('panic due to uncaught exception', filepath, file, ex);
	});
}
 ...
 */
function caPanic(str, filepath, file, err)
{
	...
	try {
		when = new Date();
		filename = 'ncore.' + file + '.' + process.pid;
		...
		if (!caPanicSkipDump) {
			caPanicLog('writing core dump to ' + filepath +
			    '/' + filename);
			caPanicSave(filepath, filename);
 ...
 */
function caPanicSave(filepath, filename)
{
	var dump = panicDbg.dump();
	var path = require('path');
	var panicDbPath = path.join(filepath, filename);

	dump = dump.replace(/,/g,",\r\n");

	mod_fs.writeFileSync(panicDbPath, dump);
}
...
function caDebugState()
{
	...
	this.set('dbg.format-version', '0.2');
